export const sidebar = {
	'/md/dataStructure/': [
		{
			text: "数据结构",
			items: [
				{text: "数据结构简介", link: "/md/dataStructure/数据结构简介"},
				{text: "列表", link: "/md/dataStructure/列表"},
				{text: "栈", link: "/md/dataStructure/栈"},
			]
		},
		{
			text: "STL容器",
			items: [
				{text: "STL 容器简介", link: "/md/dataStructure/列表"}
			]
		}
	],

	'/md/算法竞赛/算法/': [
		{
			text: "算法",
			items: [
				{text: "算法简介", link: "/md/算法竞赛/算法/算法简介"},
				{text: "常用模板", link: "/md/算法竞赛/算法/常用模板"},
				{text: "基础", link: "/md/算法竞赛/算法/基础"},
				{text: "双指针", link: "/md/算法竞赛/算法/双指针"},
				{text: "C语言程序设计高级特性", link: "/md/算法竞赛/算法/C语言程序设计高级特性"},
			]
		}
	],
	'/md/算法竞赛/题单/': [
		{
			text: "力扣",
			items: [
				{text: "力扣简介", link: "/md/算法竞赛/题单/leetCode/"},
				{text: "A+B", link: "/md/算法竞赛/题单/leetCode/A+B"},
			]
		},
		{
			text: "洛谷",
			items: [
				{text: "洛谷简介", link: "/md/算法竞赛/题单/luoGu/"},
				{text: "A+B", link: "/md/算法竞赛/题单/luoGu/A+B"},
			]
		},
		{
			text: "其它",
			items: [
				{text: "其它OJ平台", link: "/md/算法竞赛/题单/other/"},
				{text: "A+B", link: "/md/算法竞赛/题单/other/A+B"},
			]
		}
	],
	'/about/': [
		{text: "关于", link: "/about/aboutMe"},
		{text: "前言", link: "/about/"},
	]
}